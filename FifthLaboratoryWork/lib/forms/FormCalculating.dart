        ...
        child: Column(
          children: <Widget>[
            TextFormField(
              ...
              decoration: const InputDecoration(labelText: 'Введите число А:'),
            ...
            ElevatedButton(
                onPressed: () {
                  if (_formKey.currentState!.validate()) {
                    if (_agreement) {
                      final double first = double.parse(_textFieldControllerFirst.text);
                      final double second = double.parse(_textFieldControllerSecond.text);
                      BlocProvider.of<CalculationCubit>(context).calculateSum(first, second);
                    }
                    else {
                      showDialog(
                        context: context,
